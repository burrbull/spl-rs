...
/// IWDG_prescaler 
pub use crate::device::iwdg::pr::PRW as Prescaler;
...
pub trait IwdgStd {
	...
	///   IWDG_PR and IWDG_RLR registers
	fn write_access_cmd(&self, IWDG_WriteAccess : WriteAccess);
	...
	fn set_prescaler(&self, iwdg_prescaler : Prescaler);
...
impl IwdgStd for IWDG {
	fn write_access_cmd(&self, iwdg_writeaccess : WriteAccess) {
		self.kr   .write(|w| unsafe { w
			.key()   .bits( iwdg_writeaccess.into() )
		});
	}

	fn set_prescaler(&self, iwdg_prescaler : Prescaler) {
		self.pr   .write(|w| w
			.pr()   .variant( IWDG_Prescaler )
		);
	}
