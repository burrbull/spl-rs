        ...
        impl SpiStdExt for $SPIx {
            fn init(&self, spi_initstruct: SpiStruct) {
                ...
                self.cr1.modify(|_, w| w
                    .rxonly()   .bit(direction.0)
                    .bidioe()   .bit(direction.1)
                    .bidimode() .bit(direction.2)
                    .mstr()     .bit(mode.0)
                    .ssi()      .bit(mode.1)
                    .dff()      .bit(spi_initstruct.data_size == DataSize::Bit16)
                    .cpol()     .bit(spi_initstruct.cpol == ClockPolarity::IdleHigh)
                    .cpha()     .bit(spi_initstruct.cpha == ClockPhase::CaptureOnSecondTransition)
                    .ssm()      .bit(spi_initstruct.nss == SlaveSelect::Hard)
                    .br()       .bits(spi_initstruct.baudrate_prescaler as u8)
                    .lsbfirst() .bit(spi_initstruct.first_bit == FirstBit::LsbFirst)
                );
                ...
                self.crcpr
                    .write(|w| w.crcpoly().bits(spi_initstruct.crc_polynomial));
            }
