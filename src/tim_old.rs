...
pub use crate::device::tim1::ccmr1_input::IC1FW as ICFilter;
pub use crate::device::tim1::smcr::ETFW as ExtTRGFilter;
...
pub use crate::device::tim1::ccmr1_output::CC1MW as OCMode;
...
/// TIM_One_Pulse_Mode 
pub use crate::device::tim1::cr1::OPMW as OPMode;
...
pub use crate::device::tim1::cr1::CKDW as ClockDivision;
//#define tim_ckd_DIV1                       ((uint16_t)0x0000)
//#define tim_ckd_DIV2                       ((uint16_t)0x0100)
//#define tim_ckd_DIV4                       ((uint16_t)0x0200)

pub use crate::device::tim1::cr1::DIRW as CounterDirection;
pub use crate::device::tim1::cr1::CMSW as CounterAlign;
/// TIM_Counter_Mode 
...
impl CounterMode {
    pub enum to_dir_cms(&self) -> (CounterDirection, CounterAlign) {
        match {
...
/// Break_Polarity 
pub use crate::device::tim1::bdtr::BKPW as BreakPolarity;
...
/// Lock_levels 
pub use crate::device::tim1::bdtr::LOCKW as LOCKLevel;
...
macro_rules! impl_timbaseonly{
    ...
    impl  TimBaseOnlySplExt for $TIMx {
...
macro_rules! impl_timgeneral {
    ...
    impl TimGeneralSplExt for $TIMx {
        fn select_output_trigger(&self, trgo_source : TRGOSource) {
            self.cr2     .modify(|_,w| w
                .mms()   .variant( trgo_source )
            );
        ...
        fn encoder_interface_config(&self, encoder_mode : EncoderMode,
            ...
            (unsafe { self.ccmr1.input })  .modify(|_,w| w
        ...
        fn forced_oc1_config(&self, forced_action : ForcedAction) {
            (unsafe { self.ccmr1.output }) .modify(|_,w| w
                .cc1s()       .bits( 0 )
        ...
        fn forced_oc3_config(&self, forced_action : ForcedAction) {
            (unsafe { self.ccmr2.output }) .modify(|_,w| w
                .cc3s()       .bits( 0 )
        ...
        fn clear_oc4_ref(&self, oc_clear : bool) {
            (unsafe { self.ccmr4.output }) .modify(|_,w| w
                .oc4ce()  .bit( oc_clear )
            );
        }
        ...
        fn set_ic3_prescaler(&self, ic_psc : ICPrescaler) {
            (unsafe { self.ccmr2.input }) .modify(|_,w| w
        ...
        fn get_capture1(&self) -> u16 {
            self.ccr1.read().ccr1().bits()
        }

        fn get_capture2(&self) -> u16 {
            self.ccr2.read().ccr2().bits()
        }

        fn get_capture3(&self) -> u16 {
            self.ccr3.read().ccr3().bits()
        }

        fn get_capture4(&self) -> u16 {
            self.ccr4.read().ccr4().bits()
        }
...
macro_rules! impl_timadvanced {
    ...
    impl TimInitSplExt for $TIMx {

        fn bdtr_config(&self, bdtr_initstruct : &TimBDTRStruct) {
            /* Set the Lock level, the Break enable Bit and the Ploarity, the OSSR State,
                 the OSSI State, the dead time value and the Automatic Output Enable Bit */
            self.bdtr    .write(|w| unsafe { w
