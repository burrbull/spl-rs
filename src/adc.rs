...
impl AdcStruct {
    ...
    pub fn init() -> AdcStruct {
        AdcStruct {
            ...
            external_trig_conv: ExtTrigger::TIM1_CC1.into(),
...
impl From<ExtTrigger> for u8 {
    fn from(trigger: ExtTrigger) -> u8 {
        match trigger {
            ExtTrigger::TIM1_CC1 => 0,
            ExtTrigger::TIM1_CC2 => 1,
            ExtTrigger::TIM1_CC3 => 2,
            ExtTrigger::TIM2_CC2 => 3,
            ExtTrigger::TIM3_TRGO => 4,
            ExtTrigger::TIM4_CC4 => 5,
            ExtTrigger::EXTI11 => 6,
            ExtTrigger::SWSTART => 7,
        }
    }
}
...
impl From<ExtTriggerInjected> for u8 {
    fn from(trigger: ExtTriggerInjected) -> u8 {
        match trigger {
            ExtTriggerInjected::TIM1_TRGO => 0,
            ExtTriggerInjected::TIM1_CC4 => 1,
            ExtTriggerInjected::TIM2_TRGO => 2,
            ExtTriggerInjected::TIM2_CC1 => 3,
            ExtTriggerInjected::TIM3_CC4 => 4,
            ExtTriggerInjected::TIM4_TRGO => 5,
            ExtTriggerInjected::EXTI15 => 6,
            ExtTriggerInjected::JSWSTART => 7,
        }
    }
}
...
/// For ADC1 and ADC2
pub enum AdcExtTrigInjecConv12 {
    /// Timer 2 Trigger Output
    T2_TRGO = 2,
    /// Timer 2 Compare Output 1
    T2_CC1 = 3,
...
/// For ADC1, ADC2 and ADC3
pub enum AdcExtTrigInjecConv123 {
    /// Timer 1 Trigger Output
    T1_TRGO = 0,
    /// Timer 1 Compare Output 4
    T1_CC4 = 1,
...
macro_rules! impl_adc {
    ($ADCx:ty) => {
        impl AdcStdExt for $ADCx {
            ...
            fn disc_mode_channel_count_config(&self, number: u8) {
                self.cr1.modify(|_, w| w.discnum().bits(number));
            }
            ...
            fn external_trig_injected_conv_config(&self, external_trig_injec_conv: u8) {
                self.cr2
                    .modify(|_, w| w.jextsel().bits(external_trig_injec_conv));
            }
...
impl AdcInit for crate::pac::ADC1 {
    fn init(&self, adc_initstruct: &AdcStruct) {
        ...
        self.sqr1.modify(|_, w| w.l().bits(adc_initstruct.nbr_of_channel - 1));
    }
}
